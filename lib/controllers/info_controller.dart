...
class InfoController extends GetxController {
  ...
  Future<void> updateMe() async {
    if (loading.isTrue ||
        ...
        user.value.className.name.isEmpty ||
    ...
    try {
      appService.user.update((val) {
        ...
        val?.className.name = user.value.className.name;
      ...
      final _user = await UserService.updateUser(user.value.idsv,
          user.value.cccd, user.value.phone, user.value.address, user.value.department, user.value.className.name);
